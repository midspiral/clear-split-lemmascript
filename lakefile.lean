...
package ClearSplitLemmaScript where
  leanOptions := #[⟨`pp.unicode.fun, true⟩]

-- Solver download (same as lemmascript)
...
def cvc5.os := if System.Platform.isWindows then "Win64" else if System.Platform.isOSX then "macOS" else "Linux"
...
inductive Solver | z3 | cvc5
instance : ToString Solver where toString := fun | .z3 => "z3" | .cvc5 => "cvc5"
def Solver.fullName : Solver → String | .z3 => z3.fullName | .cvc5 => cvc5.fullName
def Solver.url : Solver → String | .z3 => z3.url | .cvc5 => cvc5.url

def Lake.unzip (file dir : FilePath) : LogIO PUnit := do
  IO.FS.createDirAll dir; proc (quiet := true) { cmd := "unzip", args := #["-d", dir.toString, file.toString] }
def copyFile' (src dst : FilePath) : LogIO PUnit := do
  proc (quiet := true) { cmd := "cp", args := #[src.toString, dst.toString] }
def downloadSolver (solver : Solver) (pkg : Package) (oFile : FilePath) : JobM PUnit := do
  ...
  if ← extractedPath.pathExists then IO.FS.removeDirAll extractedPath
  ...
  if ← oFile.pathExists then logInfo s!"{solver} is now at {oFile}"
  else logError s!"{solver} to {oFile}"
  IO.FS.removeFile zipPath; IO.FS.removeDirAll extractedPath
...
lean_lib Logic where
  srcDir := "src/logic"
  roots := #[`«logic.types», `«logic.def», `«logic.proof»]
